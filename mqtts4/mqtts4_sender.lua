...
local mqtts_sender = {}
local send_queue = mqtt_sender_queue.new("mqtts_4_send_queue")

mqtts_sender.TASK_NAME_PREFIX = "mqtts_4"
...
local function send_data_req_proc_func(tag, topic, payload, qos, cb)
    ...
    if dropped and dropped > 0 then
        log.warn("mqtts4_sender", "queue overflow, dropped", dropped, "remain", send_queue:length(), "bytes", send_queue:used_bytes())
    end

    if not ok then
        log.warn("mqtts4_sender", "drop payload", reason, topic, payload and #payload or 0)
        return
    end
...
local function mqtts_client_sender_task_func()
    ...
    while true do
        ...
        if msg[2] == "CONNECT_OK" then
            ...
            sys.publish("MQTT4_CONN_EVENT", true)
        ...
        elseif msg[2] == "DISCONNECTED" then
            ...
            send_item = nil
            sys.publish("MQTT4_CONN_EVENT", false)
        end
    end
end

sys.subscribe("mqtt4_send_data_req", send_data_req_proc_func)
