...
local mqtts_receiver = require("mqtts4_receiver")
local mqtts_sender = require("mqtts4_sender")
...
local SERVER_PORT = srv.port or 1883
...
local function mqtts_client_event_cbfunc(mqtt_client, event, data, payload, metas)
    log.info("mqtt4.event", event, data, payload, json.encode(metas))
...
local function wait_for_ip_ready()
    while not socket.adapter(socket.dft()) do
        log.warn("mqtt4.main", "wait IP_READY", socket.dft())
        sys.waitUntil("IP_READY", 1000)
    end
end

local function create_client()
    local mqtt_client = mqtt.create(nil, SERVER_ADDR, SERVER_PORT, false)
    if not mqtt_client then
        error_logger.error("mqtt4.main", "mqtt.create error")
        return nil
    end
    ...
    if not mqtt_client:auth(client_id, USERNAME, PASSWORD, false) then
        error_logger.error("mqtt4.main", "mqtt_client:auth error")
        mqtt_client:close()
        return nil
    end
    ...
    if not mqtt_client:connect() then
        error_logger.error("mqtt4.main", "mqtt connect failed")
        mqtt_client:close()
        return nil
    end
...
local function mqtts_client_main_task_func()
    while true do
        wait_for_ip_ready()
        log.info("mqtt4.main", "IP ready", socket.dft())
        ...
        if mqtt_client then
            while true do
                ...
                log.info("mqtt4.main", "waitMsg", msg[2], msg[3], msg[4])
