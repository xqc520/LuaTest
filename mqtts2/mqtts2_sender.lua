...
local mqtts_sender = {}
local send_queue = mqtt_sender_queue.new("mqtts_2_send_queue")
local MAX_PUBLISH_RETRIES = 2

mqtts_sender.TASK_NAME_PREFIX = "mqtts_2"
...
local function send_data_req_proc_func(tag, topic, payload, qos, cb)
    ...
    if dropped and dropped > 0 then
        log.warn("mqtts2_sender", "queue overflow, dropped", dropped, "remain", send_queue:length(), "bytes", send_queue:used_bytes())
    end

    if not ok then
        log.warn("mqtts2_sender", "drop payload", reason, topic, payload and #payload or 0)
        return
    end
...
local function try_publish_item(mqtt_client, item)
    while item do
        ...
        if not can_retry then
            log_publish_result("mqtt2.tx", item, false, nil)
            publish_item_cbfunc(item, false)
            return nil
        end

        log_retry("mqtt2.tx", item, retry_count, "publish_return_false")
    end
...
local function mqtts_client_sender_task_func()
    ...
    while true do
        ...
        if msg[2] == "CONNECT_OK" then
            ...
            retry_item = nil
            sys.publish("MQTT2_CONN_EVENT", true)
        ...
        elseif msg[2] == "PUBLISH_OK" then
            log_publish_result("mqtt2.tx", send_item, true, msg[3])
        ...
        elseif msg[2] == "DISCONNECTED" then
            ...
            if send_item then
                ...
                if can_retry then
                    log_retry("mqtt2.tx", send_item, retry_count, "disconnect_before_sent")
                    retry_item = send_item
                else
                    log_publish_result("mqtt2.tx", send_item, false, nil)
                    publish_item_cbfunc(send_item, false)
                end
            end
            mqtt_sender_queue.clear(send_queue)
            send_item = nil
            sys.publish("MQTT2_CONN_EVENT", false)
        end
    end
end

sys.subscribe("mqtt2_send_data_req", send_data_req_proc_func)
